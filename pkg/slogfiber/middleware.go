...
func NewWithConfig(logger *slog.Logger, config Config) fiber.Handler {
	...
	return func(c fiber.Ctx) error {
		...
		err := c.Next()
		if err != nil {
			if handlerErr := errHandler(c, err); handlerErr != nil {
		...
		logger.LogAttrs(c.RequestCtx(), level, msg, attributes...)
...
func extractTraceSpanID(ctx context.Context, withTraceID, withSpanID bool) []slog.Attr {
